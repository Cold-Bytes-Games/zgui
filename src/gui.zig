...
//--------------------------------------------------------------------------------------------------
var zgui_io: std.Io = undefined;
pub fn init(io_instance: std.Io, allocator: std.mem.Allocator) void {
...
pub fn initWithExistingContext(io_instance: std.Io, allocator: std.mem.Allocator, ctx: Context) void {
    zgui_io = io_instance;
    mem_allocator = allocator;
    mem_allocations = std.AutoHashMap(usize, usize).init(allocator);
    mem_allocations.?.ensureTotalCapacity(32) catch @panic("zgui: out of memory");
...
fn zguiMemAlloc(size: usize, _: ?*anyopaque) callconv(.c) ?*anyopaque {
    mem_mutex.lockUncancelable(zgui_io);
    defer mem_mutex.unlock(zgui_io);
